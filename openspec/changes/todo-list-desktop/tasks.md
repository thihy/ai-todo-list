...
- [ ] 8.11 `resources/dsh/cordis.yml` 增 sandbox 子栈：`dsh-sandbox-policy`(`mode: workspace-write`, `workspaceRoot: !!js process.env.DSH_WORKSPACE_ROOT`) + `dsh-sandbox-local`(跨平台 cordis 插件，win32 走内部 ACL runner，POSIX 走 bwrap/Landlock/Seatbelt；**不**直接挂 `dsh-sandbox-windows-acl`——它是 runner backend 库不是 cordis 插件) + `dsh-fs-sandbox`(config.cwd) + `dsh-shell-env`(config.dshHome='';提供 `ctx.shellEnv` 给下两个 shell tool) + `dsh-tool-fs` + `dsh-tool-pwsh`+`dsh-pwsh-sandbox`(win32，`disabled: !!js process.platform !== 'win32'`)/ `dsh-tool-bash`+`dsh-bash-sandbox`(POSIX) + `dsh-fs-observation-policy`(可选) + `dsh-tool-fs-search`(config `sampleOverCapGlobResults: false` 必填) + `dsh-subprocess-local`(id `subprocess`)+ `dsh-spill-local`(**不**挂抽象 `dsh-subprocess` / `dsh-spill`，否则会重复注册同名 service)；删除第 5 行 "NO bash/editor/sandbox/terminal tools" 注释。
- [ ] 8.12 `src/shared/constants.ts` 加 `DSH_WORKSPACE_SUBDIR = 'dsh_workspace'`;`src/main/index.ts` 在 `mkdirSync(attachmentsDir, ...)` 之后加 `mkdirSync(join(rootDir, DSH_WORKSPACE_SUBDIR), { recursive: true })`,并在 import DSH container **之前** `process.env.DSH_WORKSPACE_ROOT = dshWorkspaceDir`(同 DSH_SESSIONS_ROOT 处理)。
- [ ] 8.13 新建 `src/main/dsh/path-guard.ts`:导出 `isWithinWorkspace(workspace, requested)` —— `path.resolve` + `fs.realpathSync.native`(跟随 symlink/junction/长短名)+ Windows 大小写不敏感 + `real === workspace || real.startsWith(workspace + path.sep)`;配套单测覆盖 `/etc/passwd` / `../etc` / `subfolder/../notes.md` / Windows junction 越界。
- [ ] 8.14 在 `src/main/dsh/dsh-runtime.ts` 的 `bootDsh()` 内挂 `tools/pre-execute` waterfall 监听器:`read`/`read_image`/`grep`/`glob` → 调 `isWithinWorkspace` 校验,越界返回 `{ kind: 'deny', reason: 'PATH_OUTSIDE_WORKSPACE' }`,workspace 内 `await next()` 直通;`write`/`edit`/`bash`/`pwsh` → 一律返回 `{ kind: 'ask', reason: ... }` 强制审批。
- [ ] 8.15 在 `bootDsh` 现有 `ctx.on('approval/request', ...)` 监听器(行 911)最开头加短路:`aiGrantedTools[toolName] === 'always'` → resolve `'allowed-once'`;`sessionGrantsByConv.get(convId)?.has(toolName)` → resolve `'allowed-once'`;否则走现有 90s 超时 + IPC 广播逻辑。顶层维护 `sessionGrantsByConv: Map<conversationId, Set<string>>`,export `grantSessionTool(convId, toolName)` / `revokeSessionTool(convId, toolName)` 给 IPC handler 用。
- [ ] 8.16 `src/main/settings/store.ts` 加 4 处:`PersistedSettings.aiGrantedTools: Record<string, 'session' | 'always'>` + `dshWorkspaceDir: string | null`(可选覆盖,默认 `<dataDir>/dsh_workspace`);`DEFAULTS.aiGrantedTools = {}` + `DEFAULTS.dshWorkspaceDir = null`;`publicView()` 返回值加 `aiGrantedTools` + `dshWorkspaceDir`;load 自动覆盖。`src/main/ipc/ai-handlers.ts` 新增 4 个 IPC handler:`ai.userApproval.grantAlways({ reqId, toolName })`(写 settings + resolve)、`ai.userApproval.grantSession({ reqId, toolName })`(调 `grantSessionTool` + resolve)、`ai.tools.listGranted()`(返回 always 列表 + 当前会话 session 列表)、`ai.tools.revoke({ toolName, scope })`。
- [ ] 8.17 `src/renderer/dsh/PendingApprovalCard.tsx` 加 "本次会话允许" / "始终允许此工具" 按钮(在 RiskConfirmation 之外,新加 `<div className="aipane__hitl-grants">`):点击调 `window.todoList.ai.userApproval.grantSession(reqId, toolName)` / `grantAlways(...)` 再调 `onAllow()`。CSS 与 DSH RiskConfirmation 风格协调。
- [ ] 8.18 新建 `src/renderer/settings/AIGrantsPanel.tsx`(或并入现有 SettingsPanel):读 `publicView().aiGrantedTools` 列表展示 `always` 授权的工具(中文 label),每行一个 [撤销] 按钮,调 `window.todoList.ai.tools.revoke({ toolName, scope: 'always' })`。
- [ ] 8.19 `src/shared/permission-tiers.ts` 加 8 个工具名:`SAFE_TOOLS += ['read', 'read_image', 'grep', 'glob']`;`DESTRUCTIVE_TOOLS += ['write', 'edit', 'bash', 'pwsh']`。`NOTIFY_UNDO_TOOLS` 不扩展(无中间态)。`tierFor()` 不改。
- [ ] 8.20 `src/shared/tool-presentation.ts` 加 8 个工具名的 ToolName union + kindByTool + titleFor(读/查看图片/写入文件/编辑文件/运行命令/运行 PowerShell/文本搜索/文件匹配);`presentToolResult` switch case 必须返回正确 `view.card: 'read' | 'diff' | 'search' | 'terminal'`,否则渲染端退化成 JsonBlock(Plan agent O6);新建 `src/shared/tool-presentation-helpers.ts` 提供 `readResultToView` / `writeEditToView` / `grepToView` / `globToView` / `bashToView`(解析 DSH 工具返回 envelope → DSH unpack 端预制的 ReadFileLine/DiffHunk/SearchFileMatches/TerminalBlock 数据结构)。
- [ ] 8.21 修复 `src/main/ipc/ai-handlers.ts:769-792` `mutatingScope()` dead code(switch case 旧带点名字 `todo.create` → `todo_create` 等,补上实际注册的下划线名);fs/bash 工具 case 返回 `null`(**不**引入 `'fs' | 'bash'` scope,因为 dsh_workspace 内容不在 store,broadcast 无意义)。`src/shared/todo-list-api.ts:64` `DataScope` 类型不扩。
- [ ] 8.22 加 `tests/` e2e 与单元测试:`tests/dsh/path-guard.test.ts`(覆盖 /etc、..、subfolder/..、junction);`tests/main/settings/ai-grants.test.ts`(always / session 短路、revoke 失效);`tests/dsh/pre-execute.test.ts`(read 越界 deny、write 强制 ask);`tests/dsh/tool-presentation.test.ts`(read → ReadBlock、write → DiffBlock、bash → TerminalBlock 卡片类型)。
- [ ] 8.23 跨平台验证(dev 模式,文档化为 checklist):win32 跑 read/write/pwsh 三条路径;POSIX(macOS / Linux)切换 cordis.yml 为 `dsh-sandbox-local` + `dsh-bash-sandbox` 后跑同样三条路径;验证 `<dataDir>/dsh_workspace/` 创建、approval 触发、DSH sandbox path containment 生效。
