...
- [ ] 8.11 `resources/dsh/cordis.yml` 增 sandbox 子栈：`dsh-sandbox-policy`(`mode: workspace-write`, `workspaceRoot: !!js process.env.DSH_WORKSPACE_ROOT`) + `dsh-sandbox-windows-acl`(win32)/ `dsh-sandbox-local`(POSIX) + `dsh-fs-sandbox`(config.cwd) + `dsh-tool-fs` + `dsh-tool-pwsh`+`dsh-pwsh-sandbox`(win32)/ `dsh-tool-bash`+`dsh-bash-sandbox`(POSIX) + `dsh-fs-observation-policy`(可选) + `dsh-tool-fs-search` + `dsh-subprocess(-local)` + `dsh-spill(-local)`;删除第 5 行 "NO bash/editor/sandbox/terminal tools" 注释。
